...
# Note: the embedded Garage bucket is given a permissive CORS policy on `up` so a
# browser front end can read generated images from its presigned URLs. That applies to
# the local rig only - production uses Cloudflare R2, configured outside this repo.
